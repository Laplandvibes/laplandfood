...
const facts = [
  {
    ...
    body:
      "Finland's Michelin-listed restaurants are concentrated in Helsinki — every starred Finnish restaurant in recent editions of the Nordic Guide has been in the capital. Turku and Tampere have been on inspectors' radar but stars come and go.",
  },
  {
    icon: Sparkles,
    title: 'Lapland is fine-dining ambitious',
    body:
      'Lapland itself does not currently hold a Michelin star, but its premium restaurants — Nili, Aanaar, Rakas — are pushing what a tasting-menu evening looks like inside the Arctic Circle, with foraged ingredients no Helsinki kitchen can match for freshness.',
  },
  {
    icon: ChefHat,
    title: 'Reservations matter',
    body:
      'Lapland flagships are small (often 30–60 covers) and the chef-table seats fill fastest. Book peak winter (mid-Dec → Feb) and the midnight-sun window (Jun 6 → Jul 7) at least 6–8 weeks ahead. Helsinki Michelin rooms regularly have 4–6 week waits.',
  },
]
...
const laplandRooms = [
  {
    ...
    angle:
      'The most-cited fine-dining room in Lapland. Sami-rooted plates — sous-vide reindeer fillet with bilberry reduction is the signature — alongside a wild-berry programme that leans on what Inari foragers brought in that morning. Open kitchen, warm wood interior, 60 covers.',
  ...
  },
  {
    ...
    angle:
      'The closest thing to a destination dining experience deep in Sami country. Chef builds menus around Inari’s eight Sami seasons — lacto-fermented bilberries, smoked reindeer heart, cold-cured Arctic char — and the wine list is unusually deep for a town of 7,000.',
  ...
  },
  {
    ...
    angle:
      'Glass-roofed dining room facing north — fully panoramic over the spruce forest, designed so you can watch the aurora during the tasting menu in winter. Confit Arctic whitefish under wild-herb oil is the room’s signature, plated with foraged microgreens and edible flowers.',
...
const courseFlow = [
  { n: '01', label: 'Welcome bites', body: 'Two or three small bites at the table — pickled vegetables, smoked fish on rye, a foraged-herb broth shot. Sets the seasonal tone.' },
  { n: '02', label: 'Cold opener', body: 'Arctic char or whitefish in some form — cured, smoked, or as tartare. Almost always with a foraged-herb element.' },
  { n: '03', label: 'Hot starter', body: 'Mushroom or root vegetable dish in autumn; berry or herb-led plate in summer. Where chefs show off their fermentation work.' },
  { n: '04', label: 'Fish course', body: 'Pike-perch, trout, or pike, depending on what the local fishermen brought in. Cooked to centre-pink, with a butter-or-cream sauce.' },
  { n: '05', label: 'Reindeer or game', body: 'The main protein. Fillet sous-vide, slow-braised shoulder, or smoked saddle. Bilberry, lingonberry, or juniper anchor the plate.' },
  { n: '06', label: 'Cheese (optional)', body: 'Aged Finnish cheese — Lappi or Tervaleipäjuusto-style — with a tar-honey or cloudberry preserve.' },
  { n: '07', label: 'Pre-dessert', body: 'A small palate cleanser — birch-sap sorbet, cloudberry consommé, lingonberry granita.' },
  { n: '08', label: 'Dessert', body: 'Cloudberry parfait or chocolate–lingonberry dome are the two most common. Coffee comes in a copper kettle.' },
]

const bookingNotes = [
  { icon: Clock, label: 'Plan 3 hours minimum', body: 'A full Lapland tasting menu runs 2.5–3.5 hours. Don’t book a 9 PM show afterwards.' },
  { icon: Wine, label: 'Wine pairing pays off', body: 'Most Lapland flagships have a sommelier picking small-producer European wines. Pairings run €45–€95 on top of the menu.' },
  { icon: Leaf, label: 'Tell them about allergies early', body: 'Tasting menus are pre-built and bring complex foraged ingredients. Allergies / vegetarian / vegan: flag at booking, not on the night.' },
  { icon: Users, label: 'Smart casual', body: 'No restaurant in Finland will turn you away in jeans, but the rooms expect an effort. Trainers and hoodies feel out of place.' },
]

export default function MichelinDining() {
  useSEO({
    title: 'Michelin & Fine Dining in Finland and Lapland | Where to Book | LaplandFood',
    description:
      "A practical guide to Finland's Michelin-listed kitchens (Helsinki) and Lapland's three flagship fine-dining rooms (Nili, Aanaar, Rakas) — with prices, signatures, booking lead times, and what a tasting menu actually looks like.",
    path: '/michelin-dining',
  })
  ...
  return (
    <div className="min-h-screen bg-white">
      <Nav />
      <PageHero
        eyebrow="Pillar 05 · Michelin"
        title="Michelin"
        titleHighlight="& fine dining."
        subtitle="Finland's Michelin scene at a glance — what is starred, where, what is pushing fine dining inside the Arctic Circle, and how to actually book a table."
        ...
        primaryCta={{ label: 'Where to book in Lapland', href: '/michelin-dining#lapland' }}
        secondaryCta={{ label: 'Helsinki rooms', href: '/michelin-dining#helsinki' }}
      />
      ...
      <section id="helsinki" className="bg-[#F8FAFC] py-16 sm:py-20">
        <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-10">
            ...
            <AffiliateCTA
              partner="hotels"
              sid="michelin_helsinki"
              destination="Helsinki, Finland"
              className="inline-flex items-center justify-center gap-2 bg-vibe-pink hover:bg-vibe-pink/90 text-white font-semibold px-6 py-3 rounded-full transition-colors text-sm whitespace-nowrap"
            >
              Hotels in Helsinki
            </AffiliateCTA>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-5">
            {laplandRooms.map((r) => (
              <article
                key={r.name}
                className="flex flex-col rounded-2xl bg-white/5 border border-white/15 overflow-hidden hover:border-vibe-pink/40 transition-all"
              >
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <AffiliateCTA
                    partner="hotels"
                    sid={`lapland_room_${r.sid}`}
                    destination={r.city.split('·')[0].trim() + ', Finland'}
                    className="mt-auto block w-full text-center bg-vibe-pink hover:bg-vibe-pink/90 text-white font-semibold px-5 py-2.5 rounded-full transition-colors text-sm"
                  >
      ...
      <section id="stay" className="bg-white py-16">
        <div className="max-w-3xl mx-auto px-5 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center mb-6">
            ...
            <AffiliateCTA
              partner="hotels"
              sid="michelin_stay_inari"
              destination="Inari, Finland"
              className="inline-flex items-center justify-center border border-[#002F6C]/25 text-[#002F6C] hover:bg-[#002F6C]/5 font-semibold px-7 py-3.5 rounded-full transition-colors"
            >
              Hotels near Inari
            </AffiliateCTA>
          </div>
