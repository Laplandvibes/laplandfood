 ...
 *
 * NOTE: laplandfood.com origin must be on the Edge Function CORS allowlist
 * before this works in production (see project_lv_newsletter_system.md).
 */
    ...
    <section className="bg-gradient-to-br from-vibe-pink to-pink-600 py-20 sm:py-24">
      <div className="max-w-3xl mx-auto px-5 sm:px-6 lg:px-8 text-center">
        ...
        <p className="text-base sm:text-lg text-white/85 max-w-xl mx-auto mb-8">
          The seasonal recipe we’re cooking, the restaurant we just discovered, the foragers’ window we don’t want you to miss. Written by people who actually live here.
        </p>

        {status === 'success' ? (
          <div className="bg-white/15 backdrop-blur border border-white/30 rounded-2xl px-6 py-5 max-w-md mx-auto">
            <p className="text-white font-semibold">Check your inbox — welcome aboard.</p>
            <p className="text-white/80 text-sm mt-2">
              The first dispatch lands at the start of next month.
            </p>
          </div>
        ) : (
        ...
        <p className="text-white/70 text-xs mt-5">
          One email a month. Unsubscribe in one click. We never share your address.
        </p>
      </div>
    </section>
  )
